...
--             :: ([CoverTree_ exprat childC leafC dp],[CoverTree_ exprat childC leafC dp])
...
rmleaf ct = {-# SCC rmleaf #-} if stHasNoChildren (P.head childL)
    then ( P.head childL
         , ct
            { numdp    = numdp ct-nodeWeight (P.head childL)
            , children = P.tail childL
            }
         )
    else ( itrleaf
         , ct
            { numdp    = numdp ct-nodeWeight (P.head childL)
            , children = itrtree `cons` P.tail childL
            }
         )
    where
        (itrleaf,itrtree) = rmleaf $ P.head childL
        childL = children ct
-}
