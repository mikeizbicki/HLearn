...
-- 	         , Dependent MultiNormal (Replicate 200 Double)
...
class PoopTrainer x where
    type PoopDatapoint x
    pooptrain :: [PoopDatapoint x] -> x

instance 
    ( {-Monoid (Multivariate dp xs prob)
    , HomTrainer (MultivariateTF (Concat xs) prob)
    , HasDepIndex dp
    , -}
    ValueList dp  ~ (Output ': (Replicate 200 Double) )
--     (Replicate 200 Double)  ~ (Replicate 200 Double) 
--     , HList (ValueList dp) ~ HList '[] 
    ) => PoopTrainer (Multivariate dp xs prob) where
    type PoopDatapoint (Multivariate dp xs prob) = dp
    pooptrain = undefined
