...
We are given $p$ processors and $n$ tasks.
...
Then, the problem is to find a way to divvy up the students so that the sum of grades for each group is as close as possible.
We'll see some code for solving this problem in a bit.

There is another closely related problem called \prob{Bin Packing} that is easily confused with \prob{Scheduling}.
In \prob{Bin Packing}, instead of fixing the number of bins and minimizing the amount in the bins, we fix the capacity of each bin and minimize the total number of bins used.
Compare Figure \ref{fig:binpacking} below and Figure \ref{fig:scheduling} above to see the difference.
The \prob{Bin Packing} problem was originally studied by shipping companies, although like \prob{Sheduling} it occurs in many domains.
% The original motivation for the \prob{Bin Packing} problem was in shipping.
...
Both \prob{Scheduling} and \prob{Bin Packing} are \np-complete.
% Since \prob{Scheduling} is \np-complete, finding an exact solution takes exponential time.  
Therefore, I had no chance of creating an optimal grouping for my professor---the class had 100 students, and $2^{100}$ is a BIG number!
...
% We can approximately solve a \prob{Bin Packing} problem similarly to how we derived a Scheduling instance.
