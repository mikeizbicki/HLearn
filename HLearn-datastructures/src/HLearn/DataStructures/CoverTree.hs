...
prunect node = if Map.size (children' node) == 1
    then prunect $ head $ mapelemsSCC $ children' node
    else node
...
merge' !ct1 !ct2 = if distance (nodedp ct1) (nodedp ct2) > sepDist ct1
    then Nothing
        ...
        { children' = {-# SCC "go" #-} go ({-# SCC "ct1" #-} children ct1) ({-# SCC "ct2" #-} mapelemsSCC $ children ct2)
        }
    where
...
property_covering node = if Map.size (children' node) > 1 
    then maximum (map (distance (nodedp node) . nodedp) $ mapelemsSCC $ children' node) < coverDist node 
      && and (map property_covering $ mapelemsSCC $ children' node)
    else True
...
property_separation node = if Map.size (children' node) > 1
    then f $ minimum ((mapFactorial ctMaxDistance) $ mapelemsSCC $ children node) > sepChildDist node
      && and (map property_separation $ mapelemsSCC $ children' node)
    else True
    where
